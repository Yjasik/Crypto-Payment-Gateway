...
import {
  ...
  LayoutDashboard, Package, ArrowLeftRight, Settings, LogOut,
} from 'lucide-react'
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-gray-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:border-gray-800 dark:bg-gray-950/95">
      <div className="flex h-14 items-center gap-4 px-4">
              ...
              >
                {(() => {
                  ...
                  if (chain.unsupported) {
                    return <Button variant="destructive" size="sm">Wrong network</Button>
                  }
                  ...
                  return (
                    <div className="flex items-center gap-2">
                      ...
                      <Button onClick={openAccountModal} variant="outline" size="sm" className="gap-2">
                        {account.ensAvatar && (
                          <img alt="ENS Avatar" src={account.ensAvatar} className="h-4 w-4 rounded-full" />
                        )}
                        <span className="hidden md:inline">{account.displayName}</span>
                        <span className="md:hidden">{formatAddress(account.address)}</span>
        ...
        {/* Mobile user dropdown — FIXED: no asChild */}
        {isConnected && (
          <DropdownMenu>
      ...
      {isMobileMenuOpen && (
        <div className="border-t border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950 md:hidden">
          <nav className="flex flex-col space-y-2">
            <Link href="/dashboard" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => setMobileMenuOpen(false)}>
              <LayoutDashboard className="h-4 w-4" /> Dashboard
            </Link>
            <Link href="/products" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => setMobileMenuOpen(false)}>
              <Package className="h-4 w-4" /> Products
            </Link>
            <Link href="/transactions" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => setMobileMenuOpen(false)}>
              <ArrowLeftRight className="h-4 w-4" /> Transactions
            </Link>
            <Link href="/settings" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => setMobileMenuOpen(false)}>
